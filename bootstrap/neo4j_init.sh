...
BOOTSTRAPDIR="$NEOUSERHOME/repo/CSS/bootstrap"
INSTALLDIR="$NEOUSERHOME/repo/CSS/install/Server"
# specify the neo4j script install file
NEOLOG="$NEOUSERHOME/neo4j_install.log"
...
NEO4J_SERVER_FO_CONFIG_FILE=neo4j.conf.failover
# support tarballs
# SUPPORT_TGZ_FILE=neo4j_support.tar.gz - depractaed
...
STARTSCRIPT=nodestart.sh
STOPSCRIPT=nodestop.sh
CLUSTERRECONFIGSCRIPT=nodereconfig.sh
CLUSTERRESTORESCRIPT=noderestore.sh
...
#------------------------------------------------
# change to neo4j bootstrap directory
if [ -d $BOOTSTRAPDIR ]; then
	echo "located neo4j bootstrap directory"
	echo "moving to neo4j bootstrap directory"
	cd $BOOTSTRAPDIR
else
	echo "could not locate neo4j bootstrap directory"
...
if [ -d $INSTALLDIR ]; then
	echo "install repository $INSTALLDIR found"
else
	echo "unable to locate intall repostiry: $INSTALLDIR"
	echo "$(date) ERROR: Missing install repository: $INSTALLDIR." >> $NEOLOG
	logger -p local0.notice -t $LOGTAG "ERROR:  cannot locate install repository"
	exit 2
fi
...
if [ -f $NEO4J_SERVER_FO_CONFIG_FILE ]; then  
	echo "server config: $NEO4J_SERVER_FO_CONFIG_FILE found"
else
	echo "unable to locate server config: $NEO4J_SERVER_FO_CONFIG_FILE"
	echo "$(date) ERROR: Missing server config: $NEO4J_SERVER_FO_CONFIG_FILE." >> $NEOLOG
	logger -p local0.notice -t $LOGTAG "ERROR:  cannot find bootstrap files"
	exit 2
fi
...
if [ $THISNUM = 1 ]; then
	...
	echo "installing cluster node 4"
	THISH=$NODE4_HN
	THISIP=$NODE4_IP
elif [ $THISNUM = 5 ]; then
	echo "installing cluster node 5"
	THISH=$NODE5_HN
	THISIP=$NODE5_IP
elif [ $THISNUM = 6 ]; then
	echo "installing cluster node 6"
	THISH=$NODE6_HN
	THISIP=$NODE6_IP
elif [ $THISNUM = 7 ]; then
	echo "installing cluster node 7"
	THISH=$NODE7_HN
	THISIP=$NODE7_IP
elif [ $THISNUM = 8 ]; then
	echo "installing cluster node 8"
	THISH=$NODE8_HN
	THISIP=$NODE8_IP
else
...
cp $NEOUSERHOME/.bash_profile $NEOUSERHOME/.bash_profile.$(date +"%Y%m%d_%H%M%S")
...
if [ -d "/opt/neo4j/neo4j-enterprise-3.2.0" ] ; then
	export NEO4J_HOME=/opt/neo4j/neo4j-enterprise-3.2.0
fi
set -o noclobber
unset MAILCHECK
export LANG=C
...
if [ -e $NEO4J_SERVER_FO_CONFIG_FILE ]; then cp $NEO4J_SERVER_FO_CONFIG_FILE $NEOETC/$NEO4J_SERVER_FO_CONFIG_FILE && echo "NEO4J_SERVER_FO_CONFIG_FILE copied"; fi
...
if [ -d $NEOCONF ]; then
	cd $NEOCONF
	mv $NEO4J_SERVER_CONFIG_FILE $NEO4J_SERVER_CONFIG_FILE.$(date +"%Y%m%d_%H%M%S") && cp $NEOETC/$NEO4J_SERVER_CONFIG_FILE $NEO4J_SERVER_CONFIG_FILE
...
# neo4j.conf update
	...
	sed -i s/xthisip/$THISIP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode1/$NODE1_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode2/$NODE2_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode3/$NODE3_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode4/$NODE4_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode5/$NODE5_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode6/$NODE6_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode7/$NODE7_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xnode8/$NODE8_IP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	...
	sed -i s/xclustermode/$THISNODETYPE/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xclustersize/$CLUSTERSIZE/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
	sed -i s/xservergroups/$THISSERVERGROUP/g $NEO4J_SERVER_CONFIG_FILE && echo "updated $NEO4J_SERVER_CONFIG_FILE"
...
# neo4j.conf.failover update
cd $NEOETC

sed -i s/xthis_server_num/$THISNUM/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xthisip/$THISIP/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xnode1/$FONODE1/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xnode2/$FONODE2/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xnode3/$FONODE3/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xnode4/$FONODE4/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xdatadir/$NEODATA/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"	
sed -i s/xdataimport/$NEOIMPORT/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xclustermode/$THISFONODETYPE/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xclustersize/$FOCLUSTERSIZE/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"
sed -i s/xservergroups/$THISSERVERGROUP/g $NEO4J_SERVER_FO_CONFIG_FILE && echo "updated $NEO4J_SERVER_FO_CONFIG_FILE"

echo "$(date) noe4j cluister failover configuration file updated" >> $NEOLOG
logger -p local0.notice -t $LOGTAG "neo4j cluster failover configuration updated"
...
if [ -e /etc/pam.d/su ]; then
	cp /etc/pam.d/su /etc/pam.d/su.$(date +"%Y%m%d_%H%M%S")
...
if [ -e $INSTALLDIR/$BACKUP_SCRIPT ]; then
	cp $INSTALLDIR/$BACKUP_SCRIPT $NEOBIN/$BACKUP_SCRIPT
...
if [ -e $INSTALLDIR/$MONITOR_SCRIPT ]; then
	cp $INSTALLDIR/$MONITOR_SCRIPT /etc/cron.daily/$MONITOR_SCRIPT && chmod +x /etc/cron.daily/$MONITOR_SCRIPT
...
if [ -e $INSTALLDIR/$SUPPORT_SCRIPT ]; then
	cp $INSTALLDIR/$SUPPORT_SCRIPT $NEOBIN/$SUPPORT_SCRIPT && chmod +x $NEOBIN/$SUPPORT_SCRIPT
...
echo "copying failover scripts if failover configured"
echo "$(date) copying failover scripts to support bin directory" >> $NEOLOG

cd $BOOTSTRAPDIR

if [ $CONFIG_FAILOVER = "YES" ]; then
	cp $STARTSCRIPT $NEOBIN/$STARTSCRIPT && chmod +x $NEOBIN/$STARTSCRIPT
	echo "$STARTSCRIPT copied to $NEOBIN" >> $NEOLOG
	cp $STOPSCRIPT $NEOBIN/$STOPSCRIPT && chmod +x $NEOBIN/$STOPSCRIPT
	echo "$STOPSCRIPT copied to $NEOBIN" >> $NEOLOG
	cp $CLUSTERRECONFIGSCRIPT $NEOBIN/$CLUSTERRECONFIGSCRIPT && chmod +x $NEOBIN/$CLUSTERRECONFIGSCRIPT
	echo "$CLUSTERRECONFIGSCRIPT copied to $NEOBIN" >> $NEOLOG
	cp $CLUSTERRESTORESCRIPT $NEOBIN/$CLUSTERRESTORESCRIPT && chmod +x $NEOBIN/$CLUSTERRESTORESCRIPT
	echo "$CLUSTERRESTORESCRIPT copied to $NEOBIN" >> $NEOLOG
fi
